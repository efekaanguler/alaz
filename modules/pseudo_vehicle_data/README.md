...
- `/simulation/pseudo_vehicle/vehicle_speed` (`std_msgs/Float32`): Araç hızı (m/s)
- `/simulation/pseudo_vehicle/steering_angle` (`std_msgs/Float32`): Direksiyon açısı (normalize değer)

### Abone Olunan Topic'ler (`vehicle_simulator_node` için)

- `/simulation/pseudo_vehicle/throttle_cmd` (`std_msgs/Int32`): Gaz komutu (`0..100`)
- `/simulation/pseudo_vehicle/brake_cmd` (`std_msgs/Int32`): Fren komutu (`0..100`)
- `/simulation/pseudo_vehicle/steering_cmd` (`std_msgs/Float32`): Direksiyon komutu (`-1.0..1.0`)
...
```bash
ros2 run pseudo_vehicle_data vehicle_simulator_node
```

## Örnek Komut Yayını (`vehicle_simulator_node` çalışırken)

```bash
ros2 topic pub /simulation/pseudo_vehicle/throttle_cmd std_msgs/msg/Int32 "{data: 40}" -r 10
ros2 topic pub /simulation/pseudo_vehicle/brake_cmd std_msgs/msg/Int32 "{data: 0}" -r 10
ros2 topic pub /simulation/pseudo_vehicle/steering_cmd std_msgs/msg/Float32 "{data: 0.3}" -r 10
```

## İzleme

```bash
ros2 topic echo /simulation/pseudo_vehicle/vehicle_speed
ros2 topic echo /simulation/pseudo_vehicle/steering_angle
```
...
- Normal varsayılan topicler `/simulation/pseudo_vehicle/*` altında tutulur. Donanım testlerinde gerçek feedback topicleriyle karışmamalıdır.
