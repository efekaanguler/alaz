...
public:
  ...
  : Node("mission_controller")
  {
    ...
    mode_publisher_ = create_publisher<std_msgs::msg::UInt8>("/mission_control/state", 10);
...
private:
  ...
  void control_loop()
  {
    ...
    if (CURRENT_MODE != MODE_EMERGENCY && CURRENT_MODE != MODE_START &&
      !emergency_mode_->isHealthy())
    {
      last_mode_ = CURRENT_MODE;
