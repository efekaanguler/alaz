...
def generate_launch_description():
    ...
    # Load configuration - USING test_bringup.yaml FOR TESTING
    packages_cfg = _load_yaml(os.path.join(cfg_dir, 'test_bringup.yaml'))
    ...
    if not packages:
        raise RuntimeError(
            'test_bringup.yaml contains no packages. Cannot build launch graph.'
        )

    for pkg_name, pkg_cfg in packages.items():
        if not pkg_cfg.get('enabled', False):
            actions.append(LogInfo(msg=f'[test_bringup] {pkg_name}: disabled, skipping'))
            continue

        launch_file = pkg_cfg.get('launch_file')
        if not launch_file:
            raise RuntimeError(
                f"Package '{pkg_name}' is enabled but has no 'launch_file' specified in test_bringup.yaml."
            )

        # We treat pseudo_vehicle_data and dummy_camera as part of global_bringup's includes
        # if they don't have their own package directory to include from,
        # or we let them load from global_bringup/launch/includes
        if pkg_name in ["pseudo_vehicle_data", "dummy_camera", "dummy_lidar"]:
            launch_path = os.path.join(includes_dir, launch_file)
        else:
            ...
            try:
                ...
                raise RuntimeError(
                    f"Package '{pkg_name}' is enabled in test_bringup.yaml but not found in the workspace. "
                    f"Build it with 'colcon build --packages-select {pkg_name}' or disable it in test_bringup.yaml."
                )
        ...
        actions.append(LogInfo(msg=f'[test_bringup] {pkg_name}: launching {launch_file}'))
